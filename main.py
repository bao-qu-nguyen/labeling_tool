    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            if row * COLS + col >= len(files_chunk):
                ...
                img_file = cv2.imread(filepath) # read in the image with OpenCV
                ...
                btn.bind("<Button-3>", lambda event, row=row, col=col: right_click(event, row, col))
...
if __name__ == '__main__':
    root = Tk()
    root.title('LMIS Image Labeling')
